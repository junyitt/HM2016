...
netcash.v.f  <- function(teamname12, bgn1.td.df, balsh_y.df){
      netpro12 <- sapply(teamname12, FUN = function(name){
            ...
            npro1 <- sum(bgn1.td.df[u1, "netpro0"], na.rm = T); cash1 <- balsh_y.df[u2, "Cash"]
            npro1 + cash1
      })
...
#add loan if ncash < 0 
addloan.f <- function(ncash, tName, loanlength, minbalance, interest, yy){
      if(ncash < 0){
            TrackNo <- NA
            classf <- "OTC"
            FIC <- NA
...
bgn2.td.df.f <- function(netcash.v, teamname12, bgn1.td.df, loanlength = 2, minbalance = 2e6, interest = 0.12, yy){
      addloan.list <- lapply(1:length(netcash.v), FUN = function(j){
            addloan.f(netcash.v[j], teamname12[j], loanlength, minbalance, interest, yy)
      })
